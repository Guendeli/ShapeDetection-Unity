...
namespace ShapeDetector {
    ...
    public class Shape {
        ...
        public Shape(List<Vector2> points, string name = "") {
            this.Name = name;
            this.Points = points;
            this.IndicativeAngle = Shape.GetIndicativeAngle(points);
        ...
        /// comments.
        /// </summary>
        /// <param name="shapeLibrary">The library to run the gesture against.</param>
        ...
        public Result Recognize(ShapeLibrary shapeLibrary, bool useProtractor = false) {

            if (this.Points.Count <= 2) {
                return new Result("Not enough points captured", 0f);
            } else {
                List<Shape> library = shapeLibrary.Library;
                ...
                if (matchedShape == -1) {
                    ...
                    return new Result(library[matchedShape].Name, useProtractor ? 1f / bestDistance : 1f - bestDistance / this.HALF_DIAGONAL);
                }
            }
        }
        ...
        public List<Vector2> Resample(int numberOfPoints) {
            ...
            float increment = Shape.GetPathLength(this.Points) / (numberOfPoints - 1);
        ...
        public List<Vector2> RotateBy(float angle) {
            ...
            Vector2 center = Shape.GetCenter(this.Points);
        ...
        public List<Vector2> ScaleTo(float size) {
            ...
            Rect boundingBox = Shape.GetBoundingBox(this.Points);
        ...
        public static float GetIndicativeAngle(List<Vector2> points) {
            Vector2 centroid = Shape.GetCenter(points);
        ...
        public float GetDistanceAtAngle(Shape gesture, float angle) {
            List<Vector2> newPoints = this.RotateBy(angle);
            return Shape.GetDistanceBetweenPaths(newPoints, gesture.Points);
        }
        ...
        public float DollarOneAlgorithm(Shape gesture, float a, float b, float threshold) {
